...
class GaussianModel:
    ...
    def save_obj(self, path):
        ...
        scale = self._scaling.detach().cpu().numpy()
        rotation = self._rotation.detach().cpu().numpy()
        print("Shape of f_rest:", f_rest.shape)
        print(f_rest)
        with open(path, 'w') as f:
            ...
            for i in range(xyz.shape[0]):
             ...
             # 写入面（f）
            for i in range(f_dc.shape[0]):  # 假设f_dc包含三角形
                f.write(f"f {f_dc[i, 0]} {f_dc[i, 1]} {f_dc[i, 2]}\n")
    ...
    def load_ply(self, path):
        ...
        # opacities = np.random.random(plydata['vertex'].count)
        ...
        features_dc[:, 0, 0] = np.asarray(vertex_colors[0])
        features_dc[:, 1, 0] = np.asarray(vertex_colors[1])
        features_dc[:, 2, 0] = np.asarray(vertex_colors[2])
