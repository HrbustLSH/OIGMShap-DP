...
class GUI:
    def __init__(self, opt):
        ...
        if self.opt.load is not None:
            ...
            self.renderer.initialize(self.opt.pior_path)
        if self.gui:
            dpg.create_context()
            self.register_dpg()
            self.test_step()
    ...
    def save_model(self, mode='geo', texture_size=1024):
        ...
        if mode == 'geo':
            ...
            path_ply = os.path.join(self.opt.outdir, self.opt.save_path + '_model.ply')
            path_obj = os.path.join(self.opt.outdir, self.opt.save_path + '_model.obj')
            self.renderer.gaussians.save_ply(path_ply)
            self.renderer.gaussians.save_obj(path_obj)
        print(f"[INFO] save model to {path_ply}.")
        print(f"[INFO] save model to {path_obj}.")
...
if __name__ == "__main__":
    ...
    # airplane bathtub bed bench car chair cone desk guitar table
    opt.outdir =  ''
    # files = [f for f in os.listdir(inputpath) if os.path.isfile(os.path.join(inputpath, f))]
    # for img_file in files:
        # opt.input = os.path.join(inputpath, img_file)
        # opt.save_path = img_file
    
    input_init_pc_path = ''
    subfolders = [entry for entry in os.listdir(input_init_pc_path) if os.path.isdir(os.path.join(input_init_pc_path, entry))]
    
    for prompt_cate in subfolders:
        opt.prompt = 'a' + prompt_cate
        cate_path = os.path.join(input_init_pc_path,prompt_cate)
        i = 0
        for file_name in os.listdir(cate_path):
            ...
            if file_name.endswith(".ply"):
                ...
                opt.pior_path = file_path
                
                opt.save_path = f'{prompt_cate}_{str(i).zfill(4)}'
                print(f'opt.save_path:{opt.save_path}')
    # print(f'[INFO] load image from {opt.input}...')
                gui = GUI(opt)
